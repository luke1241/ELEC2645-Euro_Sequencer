...
#define CLOCK_PIN           PC_12                       //Pin definitions
...
#define MAX_SEQUENCE_LENGTH 64                          //Parameter definitions
#define DAC_SEMITONE        0.0252525252525
#define CALIBRATION_INC     0.01
#define MENU_WAIT_TIME      200ms

enum class State {                                      //Enum for states: https://www.aleksandrhovhannisyan.com/blog/finite-state-machine-fsm-tutorial-implementing-an-fsm-in-c/
    ST_IDLE,
    ST_RUN,
    ST_EDIT,
    ST_SETTINGS
...
static State g_state = State::ST_IDLE;                  //Initialise state variable to IDLE

enum Pitch {                                            //Enum of pitches, enables pitches to be reffered to by their musical names in code, rather than integer values
    c1,
    db1,
    d1,
    eb1,
...
std::string no_yes[2] = {"N", "Y"};                     //String array for converting boolean value into yes/no string output

std::string pitchStrings[36] {                          //Array of strings containing names of notes, which will be printed to the display
    "C1",
    "C#1",
    "D1",
    "D#1",
...
struct Step {                                           //Step struct hold data for each step, used instead of class as it purely stores data
...
Step sequence[MAX_SEQUENCE_LENGTH];                     //Variable declarations for sequencer
...
int selectedItem =          0;                          //Edit menu variables

int currentMenuItem =       0;                          //Settings Menu Variables
bool menuState =            0;

Controller sequenceLength_controller(sequenceLength, 8, 1, MAX_SEQUENCE_LENGTH, 1, "");     //Controller objects for settings menu
...
std::pair<Controller, std::string> settings[4] = {                                          //Array of pairs containing setting controller object and corresponding 'name' to be displayed
...
DebounceIn clockIn(CLOCK_PIN, PullDown);                //Interrupt object declarations
...
AnalogOut cvOut(CV_PIN);                                //Output object declarations
DigitalOut gateOut(GATE_PIN);
DigitalOut accentOut(ACCENT_PIN);
BusOut stateLED(STATE_LED_R, STATE_LED_G, STATE_LED_B);

N5110 lcd(PC_7, PA_9, PB_10, PB_5, PB_3, PA_10);        //LCD declaration
char lcdBuffer[14] = {0};

Joystick joystick(JOYSTICK_Y, JOYSTICK_X);              //Joystick declaration
Direction joystickDir;
DigitalIn joystick_btn(JOYSTICK_BTN, PullUp);

volatile int g_clock_flag = 0;                          //Global flags
volatile int g_run_flag =   0;

void stop_isr();                                        //ISR declarations
void run_isr();
void edit_isr();
void settings_isr();
void clock_isr();

void init_sequence();                                   //Function declarations
...
void clock_isr(){                                                   //Clock ISR
    if(g_state == State::ST_RUN){                                          
        g_clock_flag = 1;                                           //Sets clock flag high if sequence is runnning
    }                                   
}
...
void idle_state(){                                                  //Idle State
    ...
    lcd.printString("Steps:", 0, 2);                                //Printing idle screen with basic sequence information
    ...
    lcd.drawSprite(0, 0, 8, 8, (int*)pauseSmall_s);   
    lcd.drawSprite(42, 11, 37, 42, (int*)pauseBig_s);                    //Print pause sprite
...
void run_state(){                                                   //Run state
    if(g_clock_flag) {                                              //If clock pulse is received
        ...
        sprintf(lcdBuffer, "      Step %i", currStep + 1);                              //Format string to contain step number
        ...
        lcd.drawLine(0, 10, 84, 10, FILL_BLACK);                                        //Underline title
        lcd.drawSprite(42, 11, 37, 42, (int*)playBig_s);                                //Draw play sprite
        lcd.drawSprite(0, 0, 8, 8, (int*)playSmall_s);
        lcd.refresh();                                                                  //Refresh LCD to display
    ...
    if(g_run_flag) {                                                //If this is the first time running run_state()
        ...
        lcd.clear();                                                    //Clear LCD
        lcd.drawLine(0, 10, 84, 10, FILL_BLACK);                        //Display waiting message
...
void edit_state(){                                                  //Edit state                                          
    ...
    sprintf(lcdBuffer, "      Step %i", currStep + 1);                                                  //Format string with current step int
    lcd.printString(lcdBuffer, 0, 0);                                                                   //Print
    lcd.drawSprite(0, 0, 8, 8, (int*)noteSmall_s);
...
void settings_state(){                                              //Settings state
    ...
    lcd.clear();                                                    //Clear lcd
    lcd.drawSprite(0, 0, 8, 8, (int*)gearSmall_s);                  //Print title and underline
    ...
    lcd.refresh();                                                  //Refresh LCD to displa
